...
var ft;
var ajaxobj;
var httpuri = "http://" + window.location.hostname + "/";
...
function listhardware() {
  ...
  document.getElementById("doorstattype").value = config.hardware.doorstattype;
...
function uncommited() {
  ...
  document.getElementById("commit").innerHTML = "<h6>Aveti modificari nesalvate, apasati aici pentru salvarea lor si restartarea dispozitivului.</h6>";
...
function savehardware() {
  ...
  config.hardware.doorstattype = parseInt(document.getElementById("doorstattype").value);
    ...
    if (statusTxt === "success") {
      ...
      var prg = setInterval(function() {
        ...
        if (i === 101) {
          ...
          a.href = httpuri;
          a.innerText = "Reconectare la dispozitiv";
    ...
    var $modal = $("#editor-modal"),
      ...
      ft = window.FooTable.init("#usertable", {
        columns: [{
          ...
          {
            "name": "acctype",
            "title": "Access usa " + config.hardware.doorname || "1",
            "breakpoints": "xs",
            "parser": function(value) {
              if (value === 1) {
                return "Intotdeauna";
          ...
          {
            "name": "acctype2",
            "title": "Access usa " + config.hardware.relay2.doorname || "2",
            "breakpoints": "xs",
            "visible": false,
            "parser": function(value) {
              if (value === 1) {
          ...
          {
            "name": "acctype3",
            "title": "Access usa " + config.hardware.relay3.doorname || "3",
            "breakpoints": "xs",
            "visible": false,
            "parser": function(value) {
              if (value === 1) {
          ...
          {
            "name": "acctype4",
            "title": "Access usa " + config.hardware.relay4.doorname || "4",
            "breakpoints": "xs",
            "visible": false,
            "parser": function(value) {
              if (value === 1) {
        ...
        rows: data,
        editing: {
          showText: "<span class=\"fooicon fooicon-pencil\" aria-hidden=\"true\"></span> Editare utilizatori",
          addText: "Utilizator nou",
          addRow: function() {
            $editor[0].reset();
            $editorTitle.text("Adaugare utilizator nou");
            $modal.modal("show");
          },
          editRow: function(row) {
            ...
            $modal.data("row", row);
            $editorTitle.text("Editare utilizator # " + values.username);
            $modal.modal("show");
          },
          deleteRow: function(row) {
            var uid = row.value.uid;
            var username = row.value.username;
            if (confirm("Utilizatorul " + uid + " : " + username + " va fi sters din baza de date. Sunteti sigur?")) {
  ...
  if (obj.hasOwnProperty("command")) {
    switch (obj.command) {
      ...
      case "configfile":
        ...
        if (!('doorstattype' in config.hardware)) config.hardware.doorstattype = 0;
...
function clearevent() {
  if (confirm('Stergerea logului este permanenta - continuati ?')) {
...
function updateUserModalForm(){
  if(config.hardware.doorname) {
    $("#useracctype label").text("Access la " + config.hardware.doorname);
  }
  ...
  for (var i=2; i<= maxNumRelays; i++) {
    ...
    if (i<= numRelays) 
    {
      ...
      if (!(existingaccTypeForm))
      {
        ...
        $("#useracctype"+i+" label").text("Access la " + rname);
      }
    } else {
...
window.FooTable.MyFiltering = window.FooTable.Filtering.extend({
  construct: function(instance) {
    ...
    this.acctypesstr = ["Intotdeauna", "Admin", "Dezactivat"];
    this.def = config.hardware.doorname ? "Access la " + config.hardware.doorname : "Access Type";
    this.$acctype = null;
  },
  $create: function() {
    this._super();
